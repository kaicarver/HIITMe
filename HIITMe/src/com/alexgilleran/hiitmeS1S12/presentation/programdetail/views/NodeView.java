...
import com.alexgilleran.hiitmeS1S12.presentation.programdetail.EditDialogUpdateListener;
import com.alexgilleran.hiitmeS1S12.util.DraggableViewFocusListener;
import com.alexgilleran.hiitmeS1S12.util.DraggableViewTouchListener;
import com.alexgilleran.hiitmeS1S12.util.ViewUtils;

import java.util.ArrayList;
import java.util.List;

import static com.alexgilleran.hiitmeS1S12.util.ViewUtils.getBottomIncludingMargin;
import static com.alexgilleran.hiitmeS1S12.util.ViewUtils.getTopIncludingMargin;
...
public class NodeView extends LinearLayout implements DraggableView {
	...
	public InsertionPoint findInsertionPoint(int top, DraggableView viewToSwapIn) {
		...
		for (int i = FIRST_DRAGGABLE_VIEW_INDEX; i <= getLastDraggableChildIndex(); i++) {
			...
			if (top <= childView.getTop() && viewToSwapIn != null && viewToSwapIn.getParentNode() != this) {
				// in the margin above the view
				return new InsertionPoint(i, this, (DraggableView) childView, top);
			}
			if (top >= childView.getBottom() &&viewToSwapIn != null &&  viewToSwapIn.getParentNode() != this) {
